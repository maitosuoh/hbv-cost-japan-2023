...
label_mpr <- df_oral_nuc %>%
   mutate(x =  case_when(NUC =="TAF" ~ 30,
                           .default = MPR + 60/25)) %>%
  mutate(label = round(MPR, digits = 1)) %>%
...
label_nuc_aff <- df_oral_nuc %>%
  mutate(label_1 = round(AFF, digits = 2) %>% format(nsmall = 2) %>% str_remove_all("\\s")) %>%
...
```{r}
df_oral_table <- df_oral %>%
  filter(str_detect(DRUG_name, "バラクルード|エンテカビル|テノゼット|ベムリディ")) %>%
  mutate(NUC = case_when(str_detect(DRUG_name, "バラクルード")　 ~ "ETV (brand)",
                         str_detect(DRUG_name, "エンテカビル")　 ~ "ETV (generic)",
                         str_detect(DRUG_name, "テノゼット")　 ~ "TDF",
                         str_detect(DRUG_name, "ベムリディ")　 ~ "TAF"))　%>%
  distinct(DRUG_name, DRUG_code, COST_code, .keep_all = TRUE)
