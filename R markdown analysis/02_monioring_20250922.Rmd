...
label_test_cost <- df_test_cost %>%
  mutate(label_1 = round(AFF, digits = 2) ) %>%
  ...
  mutate(x1 = AFF) %>%
  mutate(hjust1 = case_when(PRO %in% c("MRI", "CT") ~ 0.5,
                            .default = 0)) %>%
  bind_cols(tibble(y2 = c(7:1) + 0.3)) %>%
  mutate(x2 = case_when(PRO %in% c("MRI") ~ AFF_2,
                       .default = 0)) %>%
  mutate(hjust2 = case_when(PRO %in% c("MRI") ~ 0.5,
                      .default = 0))
...
p1_b2 <- df_test_cost %>%
   ggplot()+
    geom_segment(aes(x = AFF_1, xend = AFF_3,
                   y = PRO, yend = PRO),
                 linewidth = 0.4) +
  geom_point(data = df_test_cost_long  %>% filter(AFF_cat %in% c("AFF", "AFF_2")),
